...
const CallDragable = ({ children, className }: PropsWithChildren & CallDragableProps) => {
  ...
  return (
    <motion.div
      ref={constraintsRef}
      className="pointer-events-none fixed inset-0 z-50 block max-h-dvh cursor-auto bg-transparent"
    >
      <motion.div
        drag
      ...
      >
        <div className={cn("flex h-full max-h-dvh w-full flex-col overflow-hidden bg-primary-100", isFullScreen ? '' : 'rounded-xl border border-primary-400')}>
          {children}
        </div>
      </motion.div>
    </motion.div>
