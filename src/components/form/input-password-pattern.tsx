      ...
      <ul className="mt-3 flex flex-col gap-3">
        {patterns?.map((pattern, index) => {
          ...
          return <li className='flex items-center gap-2' key={index}>
            <div 
              className={cn('transition-all p-1 rounded-full bg-neutral-50 inline-block text-neutral-400',
              isValid ? 'bg-green-50 text-green-700' : '')}>
                <CheckIcon size={16}/>
              {/* {isValid ? <CheckIcon size={16}/> : <XIcon size={16}/>} */}
            </div>
            <span className='text-neutral-600 font-light'>{pattern.message}</span>  
          </li>
          }
        )}
      </ul>
